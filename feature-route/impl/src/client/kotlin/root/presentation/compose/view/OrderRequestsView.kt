...
@Composable
internal fun OrderRequestsView(state: OrderRequestsState, eventHandler: (OrderRequestsEvent) -> Unit) {
    if (state.isError) {
        CommonErrorScreen { eventHandler(OrderRequestsEvent.OnRetryClick) }
    } else {
        ...
        val pullRefreshState = rememberPullRefreshState(
            ...
            onRefresh = { eventHandler(OrderRequestsEvent.OnRefreshSwipe) }
        )
                ...
                LazyColumn(
                    ...
                    item {
                        NotificationIconView(
                            ...
                            eventHandler(OrderRequestsEvent.OnNotificationsClick)
                        }
                    }
                    item {
                    ...
                    when {
                        ...
                        state.selectedCategoryFilter != null -> {
                            orderRequestItems(orderRequests = state.filteredOrderRequests, eventHandler)
                        }
                        state.orderRequests.isNotEmpty() -> {
                            orderRequestItems(orderRequests = state.orderRequests, eventHandler)
                        }
                        else -> {
                            item {
                                PlaceholderView(
...
private inline fun LazyListScope.orderRequestItems(
    ...
    crossinline eventHandler: (OrderRequestsEvent) -> Unit
) {
    ...
    itemsIndexed(orderRequests) { index, item ->
        OrderView(modifier = Modifier.padding(horizontal = 16.dp), index = index, model = item) { orderId, _ ->
            eventHandler(OrderRequestsEvent.OnOpenOrderDetailsClick(orderId))
        }
    }
}
...
@OptIn(ExperimentalMaterialApi::class)
@Composable
fun CategoriesFilterChips(state: OrderRequestsState, eventHandler: (OrderRequestsEvent) -> Unit) {
    OrderStatusCategoryUiModel.values().forEach { filter ->
        val isSelected = (state.selectedCategoryFilter == filter)
        val filteredCount = state.filteredOrderRequests.size
        val filteredCountText = if (isSelected) " ($filteredCount)" else ""
        FilterChip(
            modifier = Modifier.animateContentSize(),
            selected = isSelected,
            colors = Theme.filterChipColors,
            onClick = { eventHandler(OrderRequestsEvent.OnFilterByStatusClick(filter)) }
        ) {
            Text(
                modifier = Modifier
                    .padding(8.dp)
...
@Composable
private fun CreateNewOrderButtonView(eventHandler: (OrderRequestsEvent) -> Unit) {
    Box(
        ...
        ) { eventHandler(OrderRequestsEvent.OnCreateNewOrderClick) }
    }
}
